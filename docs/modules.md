...
| `session` | 会话表（按服务器实例隔离）、遗留进程 pid 记录（按实例分租） | `read_sessions`、`write_sessions`、`INSTANCE_ID`、`live_instance_ids`、`clear_worker_pids` |
| `worker` | 常驻会话进程、空闲回收、信号处理、孤儿清理 | `Worker`、`WORKERS`、`start_registered_worker`、`reap_orphan_workers` |
...
| `guard` | 调用护栏与用量统计、跨进程文件锁、轮次串行化 | `turn_guard`、`state_guard`、`read_state`、`log_usage`、`FileLock` |
...
| `tools` | 八个工具的 handler 与 schema | `tool_ask`、`tool_submit`、`TOOLS`、`HANDLERS`、`worker_key`、`worker_matches_intent` |
...
5. **会话进程 pid 记录按实例分租**：`workers.json` 是所有 MCP 实例共用的，读写一律走
   `core.session` 的辅助函数（`track_worker_pid` / `clear_worker_pids` / `_mutate_worker_records`），
   不要自己整表覆盖。判"孤儿"要问 `session.live_instance_ids()`（看服务器进程还在不在），
   不要用时间窗：另一条 Codex 线程正在跑长轮次时，时间窗分不出"刚被强杀"和"正忙"，
   误判会把那一轮当场杀掉。
