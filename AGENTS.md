...
- 改代码必须保持离线测试通过：`python3 -m py_compile agy_mcp.py register_agy_mcp.py` 与
...
- 版本发布：推 `v*` tag 触发 `release.yml`（先跑测试再发版）。改版本号只改 `agy_mcp.py` 的
  `SERVER_VERSION` 与 README 顶部那行。
